...
void Snake::GetTailLoc(std::vector<std::pair<short,short>> &TailLocation)
{
	std::shared_ptr<SnakeNode> currentNode = head->nextSnakeNode;
	for (int i=0; currentNode != nullptr;i++, currentNode = currentNode->nextSnakeNode)
	{
		std::pair<short, short> location(currentNode->locx, currentNode->locy);
		TailLocation[i] = std::move(location);
	}
}
